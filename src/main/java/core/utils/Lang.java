...
public class Lang {
	...
	private void changeLocale(final Locale newLocale) {
		if (locales.contains(newLocale)) {
			...
			try {
				st = new ObjectInputStream(new FileInputStream("lang/"
						+ newLocale.getLanguage() + "_"
						+ newLocale.getCountry() + ".lang"));
				lines = (HashMap<String, String>) st.readObject();
				st.close();
			} catch (Exception e) {
				e.printStackTrace();
				JOptionPane.showMessageDialog(null,
						...
						"Error", JOptionPane.ERROR_MESSAGE, new ImageIcon(
								"img/error.png"));
			}
	...
	/**
	 * @param c
	 *            Component to change language. Should be an AbstractButton or
	...
	public static void setLine(final Internationalizable c, final String key) {
		if (observed == null) {
			observed = new HashMap<Internationalizable, String>();
		}
	...
	public static Vector<String> getCombableLocales() {
		...
		final Vector<String> combo = new Vector<String>(locales.size());

		for (final Locale l : locales) {
			combo.add(Utilities.firstToUpper(l.getDisplayLanguage()) + " ("
					+ l.getDisplayCountry() + ")");
		}
